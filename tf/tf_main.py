...
model.fit(X_train, Y_train,
          ...
          shuffle=True)
sb_solution = [p.value() for p in model.trainable_weights]
...
model.fit(X_train, Y_train,
          ...
          epochs=nb_epoch,
          validation_data=(X_test, Y_test))
lb_solution = [p.value() for p in model.trainable_weights]
...
for alpha in alpha_range:
    for p in range(len(sb_solution)):
        model.trainable_weights[p].assign(lb_solution[p]*alpha +
                                             sb_solution[p]*(1-alpha))
        # model.trainable_weights[p] = lb_solution[p]*alpha +sb_solution[p]*(1-alpha)
    ...
    print("trainent:",train_xent)
    print("testent:",test_xent)
